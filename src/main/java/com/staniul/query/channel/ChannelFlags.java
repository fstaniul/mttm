 ...
 * Channel flag for inherited max family clients is not supported, and is set to 0 in {@link #flagsAsString(int)}
 */
public interface ChannelFlags {
     ...
     * It is not possible to set this property directly and that's why it is omitted in {@link #flagsAsString(int)} method.
     */
    int PASSWORD = 1 << 4;
     ...
     * reading flags.
     *
     * @param flags Flags given as a {@code int} number where each bit represents one flag.
     *
     * @return String containing flags readable by teamspeak 3 query.
     */
    static String flagsAsString(int flags) {
        return "channel_flag_permanent=" + (flags & PERMANENT) + " " +
                "channel_flag_semi_permanent=" + (flags & SEMI_PERMANENT) + " " +
                "channel_flag_maxclients_unlimited=" + (flags & MAXCLIENTS_UNLIMITED) + " " +
                "channel_flag_maxfamilyclients_unlimited=" + (flags & MAXFAMILYCLIENTS_UNLIMITED) + " " +
                "channel_flag_maxfamilyclients_inherited=0 " +
                "channel_flag_default=" + (flags & DEFAULT);
    }
    ...
    static int parseFlags(Map<String, String> channelInfo) {
        ...
        flags |= Integer.parseInt(channelInfo.get("channel_flag_password"));
        flags |= Integer.parseInt(channelInfo.get("channel_flag_default"));
